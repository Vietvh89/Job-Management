...
async function setup(options={}) {
  ...
  const client={auth:{onAuthStateChange(){},async getSession(){return{data:{session:{user:{email:'member@example.invalid'}}}};},async refreshSession(){refreshes++;return options.refreshError?{data:{session:null},error:{message:'expired'}}:{data:{session:{user:{email:'member@example.invalid'}}},error:null};},async signOut(){return{};}},rpc(name,{action,args}){
    ...
    const query={abortSignal(){return query;},then(resolve,reject){
      ...
      if(name==='jobflow_save_staff'){const error=options.staffErrors?.[staffCalls++]||options.staffError;response=error?{error}:{data:{...membership,revision:3,payload,groups:[{id:'g',name:'Test group'}],members:[]}};}
...
(async()=>{
  ...
  app=await setup({role:'admin',staffErrors:[{code:'40001',message:'stale'},null]});assert.equal((await app.window.JOBFLOW_CLOUD.saveStaff({id:'staff-test'},'g')).revision,3);assert.equal(app.writes.length,2);
  ...
  console.log('Cloud checks passed: load, serialized writes, conflict, network failure, viewer, non-member, polling keeps the editor, staff/group save, validation retry, revision refresh and session refresh.');
