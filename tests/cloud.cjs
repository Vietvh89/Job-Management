...
async function setup(options={}) {
  ...
  const membership=options.noMember?null:{email:'member@example.invalid',role:options.role||'editor'};
  const payload={jobs:[],capacity:{},timeline:{}};
  const client={auth:{onAuthStateChange(){},async getSession(){return{data:{session:{user:{email:'member@example.invalid'}}}};},async signOut(){return{};}},from(table){
    let update=false;
    const query={select(){return query;},eq(){return query;},maybeSingle(){return query;},single(){return query;},update(value){update=true;writes.push(value);return query;},abortSignal(){return query;},then(resolve,reject){
      let response=table==='jobflow_members'?{data:membership}:{data:{payload,revision:1}};
      if(update)response=options.error?{error:{message:'offline'}}:options.conflict?{data:[]}:{data:[{revision:2,updated_at:new Date().toISOString()}]};
...
(async()=>{
  ...
  assert.equal(app.window.JOBFLOW_CLOUD.save({jobs:[]}),true);assert.equal(app.window.JOBFLOW_CLOUD.save({jobs:[]}),false);await flush();assert.equal(app.writes.length,1);assert.equal(app.writes[0].revision,2);assert.equal(app.nodes.get('cloud-saving').open,false);
