(() => {
  ...
  document.addEventListener("submit",async event=>{
    ...
    try{
      ...
      refreshStaffSelects();$('#audit-panel')?.remove();render('settings');
      showToast(previous?'Staff information and Group Access saved.':'Staff member created.');
    }catch(error){
      message.textContent=error?.code==='40001'?'Data changed on the server. Keep your entered details and reload the latest data before saving again.':error?.message||'Connection interrupted. Your entries are still here. Check the connection and try again.';
      message.hidden=false;
    }finally{delete form.dataset.saving;button.disabled=false;button.textContent='Save changes';}
  });
