Warning: truncated output (original token count: 75478)
...
(() => {
  ...
  function renderTimesheets() {
    ...
    return `${pageHead("Track", "My timesheet", "Capture billable and non-billable time without leaving your workflow.", `<button class="button primary" id="add-time">${icon("plus")}Add time</button>`)}
      ...
      <div class="timesheet-grid"><section class="card"><div class="day-tabs">${[["Mon","7.5h"],["Tue","8h"],["Wed","7.25h"],["Thu","9h"],["Fri",`${logged.t…25478 tokens truncated…("x")}</button></span></div><div class="task-breakdown">${task.checklist.length?`<div class="task-checklist"><div class="nested-section-label">Task checklist</div><div class="work-check-list">${checklistMarkup(task.checklist,task.id)}</div></div>`:""}${subtasks}<div class="task-nested-actions"><button class="text-btn" data-add-subtask="${task.id}" data-job-id="${job.id}">+ Sub-task</button><button class="text-btn" data-add-task-checklist="${task.id}" data-job-id="${job.id}">+ Checklist</button></div></div></section>`;
