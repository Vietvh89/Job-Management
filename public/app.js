(() => {
  ...
  function jobsTable(jobs, compact = false) {
    ...
    return `<table class="jobs-table jobs-table-full"><thead><tr><th><input type="checkbox" data-select-all aria-label="Select all jobs"></th><th>Job number</th><th>Client</th><th>Project Description</th><th>Status</th><th>Start date</th><th>Job Completion Deadline</th><th>Progress</th><th>Priority</th><th></th></tr></thead><tbody>${jobs.map(job => {const status=jobManagerStatus(job);return `<tr data-job="${job.id}" tabindex="0"><td><input type="checkbox" data-select-job="${job.id}" ${selectedJobs.has(job.id)?"checked":""} aria-label="Select ${esc(job.name)}"></td><td><span class="job-number">${job.id}</span></td><td>${esc(job.client)}</td><td><span class="job-name">${esc(job.name)}</span><span class="job-code">Owner: ${esc(job.owner)}</span></td><td><span class="status ${statusClass(status)}">${status}</span></td><td>${shortDate(job.start)}</td><td><span class="due ${new Date(job.due) < new Date(todayKey()) && !["Completed","Cancelled"].includes(status) ? "overdue" : ""}">${shortDate(job.due)}</span></td><td><div class="progress-inline"><div class="progress-track"><span style="width:${job.progress}%"></span></div><span>${job.progress}%</span></div></td><td><span class="priority-label ${job.priority}">${job.priority}</span></td><td><button class="icon-btn" data-open-row aria-label="Open ${esc(job.name)}">${icon("chevron")}</button></td></tr>`;}).join("")}</tbody></table>`;
  }
  ...
  function todoDetailMarkup(row){
    ...
    return `<aside class="todo-detail"><div class="todo-detail-head"><span class="todo-source">${esc(row.sourceLabel)}</span><h2>${esc(row.title)}</h2><span class="todo-due ${todoDaysUntil(row.due)<0&&!row.completed?"overdue":""}">${esc(todoDeadlineLabel(row))}</span></div><div class="todo-detail-section"><strong>Reminder</strong><span>${row.reminder?new Date(row.reminder).toLocaleString():"No reminder set"}</span></div><div class="todo-detail-section"><strong>Note</strong><span class="todo-note">${row.note?esc(row.note):"No note added."}</span></div><div class="todo-detail-section"><strong>Checklist <small>${checks.filter(item=>item.completed).length}/${checks.length}</small></strong>${checks.map(item=>`<label class="todo-check-row"><input type="checkbox" data-todo-checklist="${esc(row.id)}" data-todo-check-id="${item.id}" ${item.completed?"checked":""}><span>${esc(item.text)}</span></label>`).join("")||'<span>No checklist items.</span>'}</div><div class="todo-detail-actions">${row.job?`<button class="button ghost" data-open-todo-job="${row.job.id}">Open job</button>`:""}${editable?`<button class="button ghost" data-edit-todo="${row.personal.id}">Edit task</button><button class="button danger" data-delete-todo="${row.personal.id}">Delete</button>`:""}</div></aside>`;
  }
  function renderTodo(){
  ...
  function openStaffEditor(memberId="") {
    ...
    const membership=accessMembers().find(m=>m.staff_id===member.id||(member.email&&m.email===member.email.toLowerCase()));
    const groupId=member.email?(membership?.group_id??member.accessGroupId??''):(member.accessGroupId??membership?.group_id??'');
    showPanel(member.id?"Edit staff member":"Add staff member",`<form id="staff-form"><input type="hidden" name="id" value="${esc(member.id)}"><label>Staff ID<input name="staffId" value="${esc(member.staffId)}" required maxlength="40"></label><label>Full name<input name="fullName" value="${esc(member.fullName)}" required maxlength="160"></label><label>Short name<input name="shortName" value="${esc(member.shortName)}" required maxlength="40"><small>Displayed in Job Manager, Schedule and capacity views.</small></label><label>Rank<input name="rank" value="${esc(member.rank)}" required maxlength="30"></label><label>Department<input name="department" value="${esc(member.department)}" required maxlength="80"></label><label>Position / Role<input name="role" value="${esc(member.role)}" required maxlength="120"></label><label>Status<select name="status"><option value="active" ${member.status!=="inactive"?"selected":""}>Active</option><option value="inactive" ${member.status==="inactive"?"selected":""}>Inactive</option></select></label><label>Email<input name="email" type="email" value="${esc(member.email||"")}" maxlength="254" placeholder=""><small>Use the user’s sign-in email to link access.</small></label><label>Group Access<select name="groupId" ${membership?.role==="admin"?"disabled":""}><option value="">None</option>${accessGroups().map(g=>`<option value="${g.id}" ${g.id===groupId?"selected":""}>${esc(g.name)}</option>`).join("")}</select><small>${membership?.role==="admin"?"Account Owners keep full access.":"Group permissions apply to this user. With no email, the group is saved until an email is added. Individual overrides in Privileges remain unchanged."}</small></label><label>Target utilization (%)<input name="targetUtilization" type="number" min="1" max="150" step="1" value="${member.targetUtilization}" required></label>${window.JOBFLOW_CLOUD?`<fieldset><legend>Login password</legend><label>New password<input name="newPassword" type="password" minlength="8" maxlength="72" autocomplete="new-password"></label><label>Confirm password<input name="confirmPassword" type="password" minlength="8" maxlength="72" autocomplete="new-password"></label><small>Leave both fields blank to keep the current password. If this email has no login account, setting a password creates one.</small></fieldset>`:""}<p id="staff-form-message" role="alert" hidden></p><button class="button primary" type="submit">${member.id?"Save changes":"Add staff member"}</button></form>`);
  }

  function staffMemberDraft(data){
    const fields=['staffId','fullName','shortName','rank','department','role'];
    const draft=Object.fromEntries(fields.map(key=>[key,String(data[key]||'').trim()]));
    if(fields.some(key=>!draft[key]))throw new Error('Complete all required staff fields.');
    if(state.capacity.members.some(m=>m.id!==data.id&&(m.staffId.toLowerCase()===draft.staffId.toLowerCase()||m.shortName.toLowerCase()===draft.shortName.toLowerCase())))throw new Error('Staff ID and short name must be unique.');
    draft.email=String(data.email||'').trim().toLowerCase();
    if(draft.email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(draft.email))throw new Error('Enter a valid email address.');
    if(draft.email&&state.capacity.members.some(m=>m.id!==data.id&&m.email?.toLowerCase()===draft.email))throw new Error('Email must be unique.');
    draft.targetUtilization=Number(data.targetUtilization);
    if(!Number.isFinite(draft.targetUtilization)||draft.targetUtilization<1||draft.targetUtilization>150)throw new Error('Target utilization must be between 1% and 150%.');
    return {...draft,id:data.id||crypto.randomUUID(),status:data.status==='inactive'?'inactive':'active'};
  }
  function applyStaffDraftLocally(draft){
    let member=state.capacity.members.find(m=>m.id===draft.id),previous=member?.shortName;
    if(!member){member={id:draft.id,weeklyHours:40,workDays:[1,1,1,1,1],allocations:[],leaves:[]};state.capacity.members.push(member);}
    Object.assign(member,draft,{name:draft.shortName,team:draft.department,code:staffCode(draft.shortName)});
    if(previous&&previous!==draft.shortName){
      for(const job of state.jobs){if(job.owner===previous)job.owner=draft.shortName;job.team=job.team?.map(name=>name===previous?draft.shortName:name)||[];}
      for(const project of state.timeline.projects)for(const item of project.items){if(item.owner===previous)item.owner=draft.shortName;for(const subtask of item.subtasks||[])if(subtask.owner===previous)subtask.owner=draft.shortName;}
      for(const item of state.recurring||[])if(item.owner===previous)item.owner=draft.shortName;
    }
  }
  function refreshStaffSelects() {
    ...
    for(const selector of ['#recurring-form [name="owner"]','#phase-owner','#phase-task-owner']){const select=$(selector);if(select)select.innerHTML=options;}
  }
  ...
  function refreshClientSelects() {
    ...
    const recurringSelect=$("#recurring-form [name=\"client\"]");
    if(recurringSelect)recurringSelect.innerHTML=options;
  }

  function newJobPickerCandidates(kind) {
    if(kind==="client")return [...new Set(state.jobs.map(job=>job.client))].sort().map(client=>({value:client,label:client,meta:`${state.jobs.filter(job=>job.client===client).length} jobs`,terms:[client]}));
    if(kind==="owner")return state.capacity.members.filter(member=>member.status!=="inactive").map(member=>({value:member.shortName,label:`${member.shortName} · ${member.fullName}`,meta:[member.staffId,member.rank,member.department].filter(Boolean).join(" · "),terms:[member.staffId,member.shortName,member.fullName,member.email,member.rank,member.department,member.role]}));
    return [];
  }

  function newJobPickerMatches(kind,query) {
    const q=String(query||"").trim().toLowerCase();
    if(!q)return [];
    return newJobPickerCandidates(kind).filter(item=>item.terms.some(term=>String(term||"").toLowerCase().includes(q))).slice(0,10);
  }

  function renderNewJobPickerSuggestions(kind,query) {
    const list=$(`#new-job-${kind}-results`);if(!list)return;
    const matches=newJobPickerMatches(kind,query);list.hidden=!String(query||"").trim();
    list.innerHTML=matches.length?matches.map(item=>`<button type="button" class="client-suggestion" role="option" data-new-job-picker-select="${kind}" data-new-job-picker-value="${esc(item.value)}" data-new-job-picker-label="${esc(item.label)}"><span><strong>${esc(item.label)}</strong><small>${esc(item.meta)}</small></span><span>Select ${icon("chevron")}</span></button>`).join(""):`<div class="client-suggestion-empty">No ${kind==="client"?"clients":"staff"} contain “${esc(String(query||"").trim())}”</div>`;
  }

  function selectNewJobPicker(kind,value,label) {
    const form=$("#job-form"),search=$(`#new-job-${kind}-search`),hidden=form?.elements[kind];
    if(!form||!search||!hidden||!newJobPickerCandidates(kind).some(item=>item.value===value))return false;
    hidden.value=value;search.value=label||value;search.setAttribute("aria-invalid","false");
    const list=$(`#new-job-${kind}-results`);if(list)list.hidden=true;
    return true;
  }
  ...
  function prepareNewJobForm(recurring=false) {
    ...
    $("#new-job-client-search").value="";
    const defaultOwner=state.capacity.members.find(member=>member.status!=="inactive");
    form.elements.owner.value="";
    $("#new-job-owner-search").value="";
    if(defaultOwner)selectNewJobPicker("owner",defaultOwner.shortName,`${defaultOwner.shortName} · ${defaultOwner.fullName}`);
    for(const kind of ["client","owner"]){const list=$(`#new-job-${kind}-results`);if(list){list.hidden=true;list.innerHTML="";}}
  ...
  document.addEventListener("submit",async event=>{
    ...
    const form=event.target;if(form.dataset.saving)return;
    const message=form.querySelector('#staff-form-message'),button=form.querySelector('[type="submit"]');
    let staffSaved=false,passwordAttempted=false;
    try{
      const data=Object.fromEntries(new FormData(form)),member=staffMemberDraft(data);
      const newPassword=String(data.newPassword||''),confirmPassword=String(data.confirmPassword||'');
      if(newPassword||confirmPassword){
        if(!member.email)throw new Error('Set a valid Staff email before creating or changing a login password.');
        if(newPassword.length<8||newPassword.length>72)throw new Error('Password must contain between 8 and 72 characters.');
        if(newPassword!==confirmPassword)throw new Error('Password confirmation does not match.');
      }
      const previous=state.capacity.members.find(m=>m.id===member.id);
      const access=accessMembers().find(m=>m.staff_id===member.id||(previous?.email&&m.email===previous.email.toLowerCase()));
      const groupId=access?.role==='admin'?access.group_id||'':data.groupId||'';
      if(member.email&&groupId!==(access?.group_id??previous?.accessGroupId??'')&&!window.confirm('Save this staff member and update their Group Access? Individual permission overrides will be kept.'))return;
      member.accessGroupId=groupId;
      if(form.elements.id)form.elements.id.value=member.id;
      message.hidden=true;form.dataset.saving='true';button.disabled=true;button.textContent='Saving…';
      if(window.JOBFLOW_CLOUD){
        const result=await window.JOBFLOW_CLOUD.saveStaff(member,groupId);
        state=structuredClone(result.payload);state.capacity.members.forEach(m=>{m.status??='active';m.email??='';});
        syncRelations();lastSavedState=JSON.stringify(state);privilegeDraft=null;
        staffSaved=true;
        if(newPassword){passwordAttempted=true;await window.JOBFLOW_CLOUD.setStaffPassword(member.email,newPassword);}
      }else{
        const before=structuredClone(state);applyStaffDraftLocally(member);
        if(!save()){state=before;throw new Error('Could not save staff changes.');}
      }
      refreshStaffSelects();settingsSection='staff';render('settings');
      message.textContent=newPassword?'Saved successfully. Staff information, access and login password are now updated.':previous?'Saved successfully. Staff information and Group Access are now updated.':'Saved successfully. The staff member has been created.';message.hidden=false;
      showToast(previous?'Staff information and Group Access saved.':'Staff member created.');
    }catch(error){
      message.textContent=staffSaved?`Staff information was saved, but the login password was not changed. ${error?.message||'Try entering the password again.'}`:(error?.code==='PT409'||error?.status===409)?'Data changed again while saving. Your entries are still here; press Save changes once more.':error?.message||'Connection interrupted. Your entries are still here. Check the connection and try again.';
  ...
  document.addEventListener("click",event=>{
    ...
    if(event.target.closest("[data-new-todo-list]")){openTodoList();return;}
    const editList=event.target.closest("[data-edit-todo-list]");if(editList){openTodoList(editList.dataset.editTodoList);return;}
  ...
  function handleTodoSubmit(event){
    ...
    if(event.target.id==="todo-task-form"){
      ...
      const values={title,listId:String(data.listId||""),due:String(data.due||""),reminder:String(data.reminder||""),note:String(data.note||"").trim(),priority:String(data.priority||"medium"),checklist};
  ...
  window.JOBFLOW_SAVE_TODO=form=>handleTodoSubmit({target:form,preventDefault(){},stopPropagation(){}});
  ...
  function renderJobOverview(job) {
    ...
    return `<section class="job-overview-grid">
      ...
      <article class="overview-card margin-card"><span>Job Completion Deadline</span><strong>${shortDate(job.due)}</strong><small class="${remainingDays===0?"danger-text":"on-track"}">${remainingDays} days remaining</small></article>
  ...
  document.addEventListener("click", event => {
    ...
    const newJobPickerSuggestion=event.target.closest("[data-new-job-picker-select]");
    if(newJobPickerSuggestion)selectNewJobPicker(newJobPickerSuggestion.dataset.newJobPickerSelect,newJobPickerSuggestion.dataset.newJobPickerValue,newJobPickerSuggestion.dataset.newJobPickerLabel);
    ...
    if (event.target.closest("[data-display-options]")) showPanel("Job display options",`<form id="job-display-form">${["Client","Start date","Job Completion Deadline","Progress","Priority"].map((label,i)=>`<label><input type="checkbox" name="col-${[3,6,7,8,9][i]}" ${state.hiddenJobColumns?.includes([3,6,7,8,9][i])?"":"checked"}> ${label}</label>`).join("")}<button class="button primary">Apply columns</button></form>`);
  ...
  document.addEventListener("input", event => {
    if(event.target.matches("[data-new-job-picker]")) {
      const kind=event.target.dataset.newJobPicker,hidden=$("#job-form")?.elements[kind];
      if(hidden)hidden.value="";
      event.target.setAttribute("aria-invalid","false");
      renderNewJobPickerSuggestions(kind,event.target.value);
    }
  ...
  document.addEventListener("keydown", event => {
    if(event.target.matches("[data-new-job-picker]")&&event.key==="Enter"){
      const kind=event.target.dataset.newJobPicker,first=newJobPickerMatches(kind,event.target.value)[0];
      if(first){event.preventDefault();selectNewJobPicker(kind,first.value,first.label);}return;
    }
  ...
  document.addEventListener("submit", event => {
    if (event.target.matches("#job-info-form")) {
      ...
      if (!data.start || !data.due || data.due < data.start) {
        showToast("Job Completion Deadline must be on or after the start date.");
        return;
      }
      const project=ensureJobPhaseHierarchy(job);
  ...
  $("#job-form").addEventListener("submit", event => {
    event.preventDefault();
    const data = Object.fromEntries(new FormData(event.target));
    for(const kind of ["client","owner"]){if(!data[kind]){const search=$(`#new-job-${kind}-search`);search?.setAttribute("aria-invalid","true");search?.focus();showToast(`Search and select a valid ${kind==="client"?"client":"staff member"}.`);return;}}
    let newJob;
