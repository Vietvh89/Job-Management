-- Staff and Group Access are committed together. No direct table grants or elevated wrapper.
create function jobflow_private.rename_staff_owner(doc jsonb,previous_name text,next_name text)
returns jsonb language plpgsql immutable security invoker set search_path='' as $$
declare result jsonb; entry record;
begin
 if previous_name is null or previous_name=next_name then return doc; end if;
 if jsonb_typeof(doc)='array' then
  select coalesce(jsonb_agg(jobflow_private.rename_staff_owner(value,previous_name,next_name)),'[]') into result from jsonb_array_elements(doc);
  return result;
 elsif jsonb_typeof(doc)='object' then
  result:='{}';
  for entry in select key,value from jsonb_each(doc) loop
   if entry.key='owner' and entry.value=to_jsonb(previous_name) then result:=result||jsonb_build_object(entry.key,next_name);
   elsif entry.key='team' and jsonb_typeof(entry.value)='array' then
    result:=result||jsonb_build_object(entry.key,(select coalesce(jsonb_agg(case when value=to_jsonb(previous_name) then to_jsonb(next_name) else value end),'[]') from jsonb_array_elements(entry.value)));
   else result:=result||jsonb_build_object(entry.key,jobflow_private.rename_staff_owner(entry.value,previous_name,next_name)); end if;
  end loop;
  return result;
 end if;
 return doc;
...
create function public.jobflow_save_staff(args jsonb) returns jsonb
...
begin
 -- This entry point inherits the existing live-session/membership check and advisory lock.
 ...
 if (args->>'revision')::int is distinct from (ctx->>'revision')::int then raise exception 'Data changed on the server. Reload before saving.' using errcode='40001'; end if;
 ...
 -- Blank email keeps the group as a draft; it grants no account access.
